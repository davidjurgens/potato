...
For Codex, Cursor and other agents, the
[`skills`](https://github.com/vercel-labs/skills) CLI installs it:

```bash
npx skills add davidjurgens/potato-skill --agent codex cursor
```

`codex` and `cursor` put the skill in `.agents/skills/potato-skill/`;
`claude-code` puts it in `.claude/skills/potato-skill/`. If your agent reads
`AGENTS.md` and nothing else, copy `AGENTS.md` from the root of that repository
into your project.

Potato has to be installed where the agent runs commands, because the skill's
helper scripts import it and call the `potato` CLI.
