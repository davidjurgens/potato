...
Pocket Mode hosts the simplest types: one card per item and one tap per
label. Many more types work on a phone through the regular annotation page,
which is laid out for phones (see [Phones without Pocket
Mode](#phones-without-pocket-mode)). Only types that have not been checked by
touch get a warning recommending a desktop browser. The admin dashboard shows
who is annotating from which kind of device either way.
...
| Device | Pocket-capable task, Pocket enabled | Other task, every type works by touch | Task with a type not checked for touch |
|---|---|---|---|
| Desktop | normal interface | normal interface | normal interface |
| Phone / tablet | **redirected to the mobile interface** | normal interface, laid out for the phone | normal interface + a dismissible warning that a desktop browser is recommended |
...
### Phones without Pocket Mode

A phone that is not routed to Pocket gets the regular annotation page, laid
out for a small screen:

- The header takes two rows and scrolls away with the page. Long task names
  are shortened, with the full name on hover.
- The task scrolls with the page. Only PDF and document viewers keep their
  own scrolling box.
- Options, Likert-scale points, ranking arrows, slider handles and image
  toolbar buttons are at least 44px to touch. Multirate grids become a list of
  items, each with its ratings as rows.
- Keyboard-shortcut hints are hidden.
- **Span highlighting:** select text with a long press, then tap the
  **Highlight as "label"** button that appears at the bottom of the screen.
  Choose the label first, or the button asks for one.

These types work on the regular page by touch: `radio`, `multiselect`,
`likert`, `slider`, `number`, `text`, `pure_display`, `select`, `multirate`,
`pairwise`, `bws`, `ranking`, `semantic_differential`, `span` and
`image_annotation`. A task with any other type shows phone and tablet users a
dismissible warning recommending a desktop browser. The warning scrolls away
with the page.
...
Why this matters: if your task includes a type not checked for touch and the
Devices tab shows annotators on phones anyway, those annotators saw the
warning and kept going. Check their output or follow up with them. Device data also
