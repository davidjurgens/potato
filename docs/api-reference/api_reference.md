...
Nothing is stored for it. That is the shape annotations are *stored* in, which
makes it the natural first guess. A request where no key can be read is
refused with `400` listing the keys it could not read.
