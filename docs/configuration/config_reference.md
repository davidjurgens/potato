...
| Key | Required | Type | Default | Description | Sub-keys |
|-----|----------|------|---------|-------------|----------|
| `adjudication` |  | object |  | Resolve disagreements through an adjudication queue | `adjudicator_users`, `agreement_threshold`, `enabled`, `error_taxonomy`, `fast_decision_warning_ms`, `include_machine_annotators`, `min_annotations`, `min_human_annotations`, `output_subdir`, `require_confidence`, `require_notes_on_override`, `show_agreement_scores`, `show_all_items`, `show_annotator_names`, `show_timing_data`, `similarity` |
...
| `ai_budget` |  | object |  | Cost estimate and spend cap for AI actions. The complaint about commercial platforms is not the price but the surprise -- credits consumed by auto-labelling and discovered at export time | `cap_usd`, `prices` |
...
| `psychometrics` |  | object |  | Live IRT. Fits item difficulty and annotator ability as labels arrive, so labels carry error bars and confident items can stop early | `confidence_threshold`, `cost_per_judgment`, `discrimination_flag_threshold`, `enabled`, `include_machine_annotators`, `min_annotators_per_item`, `min_observations`, `refit_interval`, `schema` |
