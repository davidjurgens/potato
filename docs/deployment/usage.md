...
- Specify the [annotation schema](../annotation-types/schemas_and_templates.md), including:
...
- Choose an existing [HTML template](../annotation-types/schemas_and_templates.md#available-layout-templates) for the annotation task or create a new one
- Optional: Specify privacy and access [settings](../auth-users/user_and_collaboration.md) for the task
- Optional: Update the YAML file with the [UI and layout configuration](../configuration/configuration.md#ui-and-layout-configuration)
- Optional: Set up [active learning](../ai-intelligence/active_learning_guide.md)
...
This will launch the webserver on port 8000 which can be accessed at <http://localhost:8000>. You can [create an account](../auth-users/user_and_collaboration.md) and start labeling data. Clicking "Submit" will auto-advance to the next instance and you can navigate between items using the arrow keys. Potato currently supports one annotation task per server instance, though multiple servers may be run on different ports to concurrently annotate different data.
...
See [Password Management](../auth-users/password_management.md) for more details on password reset flows.
...
See [Migration CLI](../tools/migration_cli.md) for details.
