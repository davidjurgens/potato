...
def user_name_endpoint():
    ...
    # ## SKY: I commented this out because it was causing errors
...
def generate_schematic(annotation_scheme):
    ...
    if annotation_type == "multiselect":      
        ...
        schematic += 'var sliderDiv = document.getElementById("sliderValue_' + "".join(annotation_scheme['name'].split()) + '");\n'
        for i, label_data in enumerate(annotation_scheme['labels']):
            label = label_data if isinstance(label_data, str) else label_data['name']
            schematic += 'if (slideAmount == ' + str(i) + ') {\nsliderDiv.innerHTML = "' + label + '";\n'
            schematic += 'document.getElementById("' + "".join(annotation_scheme['name'].split()) + '").setAttribute("value", slideAmount.toString());}\n'
        schematic += "\n}\n</script>"
