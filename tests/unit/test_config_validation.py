...
class TestConfigValidation:
    ...
    def temp_project_dir(self):
        ...
        # Copy data files from test-configs/data to temp/data
        source_data_dir = os.path.join(test_configs_dir, 'data')
        if os.path.exists(source_data_dir):
            for file_name in os.listdir(source_data_dir):
                source_file = os.path.join(source_data_dir, file_name)
    ...
    def test_required_fields_validation(self, temp_project_dir):
        ...
        config_files = [
            'tests/test-configs/likert-annotation.yaml',
            'tests/test-configs/multiselect-annotation.yaml',
            'tests/test-configs/slider-annotation.yaml',
            'tests/test-configs/span-annotation.yaml',
            'tests/test-configs/multirate-annotation.yaml',
            'tests/test-configs/text-annotation.yaml',
            'tests/test-configs/mixed-annotation.yaml'
        ]
    ...
    def test_data_files_validation(self, temp_project_dir):
        ...
        for config_file in config_files:
            ...
            for data_file in data_files:
                full_path = os.path.join(temp_project_dir, data_file)
    ...
    def test_phases_validation(self, temp_project_dir):
        """Test that phases configuration is valid."""
        config_files = [
            'tests/test-configs/mixed-annotation.yaml'  # This one has phases
        ]
        ...
        for config_file in config_files:
            ...
            if 'phases' in config_data:
                ...
                assert isinstance(phases, dict), f"phases must be a dict in {config_file}"

                # Check that phases have required structure
                for phase_name, phase_config in phases.items():
                    assert isinstance(phase_config, dict), f"Phase config must be dict in {config_file}"
                    assert 'type' in phase_config, f"Missing type in phase {phase_name} in {config_file}"

                    # Validate phase type
                    valid_types = ['consent', 'instructions', 'annotation']
                    assert phase_config['type'] in valid_types, f"Invalid phase type in {config_file}"

                    # Check for file if present
                    if 'file' in phase_config:
                        file_path = os.path.join(temp_project_dir, phase_config['file'])
                        assert os.path.exists(file_path), f"Phase file not found: {file_path}"
    ...
    def test_template_validation(self, temp_project_dir):
        ...
        for config_file in config_files:
            ...
            # Check that site_dir is present and valid
            assert 'site_dir' in config_data, f"Missing site_dir in {config_file}"
            assert isinstance(config_data['site_dir'], str), f"site_dir must be string in {config_file}"
    ...
    def test_alert_time_validation(self, temp_project_dir):
        ...
        for config_file in config_files:
            ...
            assert alert_time >= 0, f"alert_time_each_instance must be non-negative in {config_file}"
...
class TestConfigIntegration:
    ...
    def test_config_with_real_data_files(self, temp_project_dir):
        ...
        data_file = os.path.join(temp_project_dir, 'data', 'test_data.json')
