"""Server integration tests for soft_label annotation."""
...
def create_soft_label_config(test_dir, data_file, port=9030):
    """Create a soft_label test config."""
    abs_test_dir = os.path.abspath(test_dir)
    output_dir = os.path.join(abs_test_dir, "output")
    os.makedirs(output_dir, exist_ok=True)

    config = {
        "annotation_task_name": "Soft Label Test",
        ...
        "port": port,
        "host": "0.0.0.0",
        "secret_key": "test-secret-key-soft-label",
...
class TestSoftLabelServer:
    """Integration tests for soft_label annotation server."""

    @pytest.fixture(scope="class", autouse=True)
    def flask_server(self, request):
        """Start a Flask server with soft_label config."""
        test_dir = create_test_directory("soft_label_server_test")
        data_file = create_test_data(test_dir)
        config_path = create_soft_label_config(test_dir, data_file, port=9030)

        server = FlaskTestServer(port=9030, config_file=config_path)
        if not server.start():
            cleanup_test_directory(test_dir)
            pytest.fail("Failed to start soft_label test server")
    ...
    def _get_session(self):
        ...
        username = f"soft_label_tester_{uuid.uuid4().hex[:6]}"
        session.post(
    ...
    def test_submit_annotation(self):
        ...
        response = session.post(
            ...
            json={
                "instance_id": "1",
                "annotations": {
                    "sentiment_dist:Positive": "40",
                    "sentiment_dist:Neutral": "35",
                    "sentiment_dist:Negative": "25",
                },
            },
        )
        assert response.status_code == 200
    ...
    def test_submit_and_verify_response(self):
        ...
        response = session.post(
            ...
            json={
                "instance_id": "1",
                "annotations": {
                    "sentiment_dist:Positive": "50",
                    "sentiment_dist:Neutral": "30",
                    "sentiment_dist:Negative": "20",
                },
            },
        )
        assert response.status_code == 200
    ...
    def test_annotate_page_contains_labels(self):
        """Annotation page contains the configured label names."""
        session = self._get_session()
        response = session.get(f"{self.server.base_url}/annotate")
        assert response.status_code == 200
