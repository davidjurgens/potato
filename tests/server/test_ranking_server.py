"""Server integration tests for ranking annotation."""
...
def create_ranking_config(test_dir, data_file, port=9034):
    """Create a ranking test config."""
    abs_test_dir = os.path.abspath(test_dir)
    output_dir = os.path.join(abs_test_dir, "output")
    os.makedirs(output_dir, exist_ok=True)

    config = {
        "annotation_task_name": "Ranking Test",
        ...
        "port": port,
        "host": "0.0.0.0",
        "secret_key": "test-secret-key-ranking",
...
class TestRankingServer:
    """Integration tests for ranking annotation server."""

    @pytest.fixture(scope="class", autouse=True)
    def flask_server(self, request):
        """Start a Flask server with ranking config."""
        test_dir = create_test_directory("ranking_server_test")
        data_file = create_test_data(test_dir)
        config_path = create_ranking_config(test_dir, data_file, port=9034)

        server = FlaskTestServer(port=9034, config_file=config_path)
        if not server.start():
            cleanup_test_directory(test_dir)
            pytest.fail("Failed to start ranking test server")
    ...
    def _get_session(self):
        ...
        username = f"ranking_tester_{uuid.uuid4().hex[:6]}"
        session.post(
    ...
    def test_server_starts(self):
        """Server starts successfully with ranking config."""
        response = requests.get(f"{self.server.base_url}/")
        assert response.status_code == 200

    def test_annotate_page_loads(self):
        """/annotate returns 200 with ranking form HTML."""
    ...
    def test_annotate_page_contains_labels(self):
        """Annotation page contains the configured ranking labels."""
        session = self._get_session()
        response = session.get(f"{self.server.base_url}/annotate")
        assert response.status_code == 200
        # All three labels should appear in the ranking list
