"""Server integration tests for constant_sum annotation."""
...
def create_constant_sum_config(test_dir, data_file, port=9032):
    """Create a constant_sum test config."""
    abs_test_dir = os.path.abspath(test_dir)
    output_dir = os.path.join(abs_test_dir, "output")
    os.makedirs(output_dir, exist_ok=True)

    config = {
        "annotation_task_name": "Constant Sum Test",
        ...
        "port": port,
        "host": "0.0.0.0",
        "secret_key": "test-secret-key-constant-sum",
...
class TestConstantSumServer:
    """Integration tests for constant_sum annotation server."""

    @pytest.fixture(scope="class", autouse=True)
    def flask_server(self, request):
        """Start a Flask server with constant_sum config."""
        test_dir = create_test_directory("constant_sum_server_test")
        data_file = create_test_data(test_dir)
        config_path = create_constant_sum_config(test_dir, data_file, port=9032)

        server = FlaskTestServer(port=9032, config_file=config_path)
        if not server.start():
            cleanup_test_directory(test_dir)
            pytest.fail("Failed to start constant_sum test server")
    ...
    def _get_session(self):
        ...
        username = f"constant_sum_tester_{uuid.uuid4().hex[:6]}"
        session.post(
    ...
    def test_annotate_page_contains_labels(self):
        ...
        response = session.get(f"{self.server.base_url}/annotate")
        assert response.status_code == 200
        assert "topic_alloc" in response.text
