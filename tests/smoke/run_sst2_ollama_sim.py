#!/usr/bin/env python3
"""One-off runner for SST-2 hybrid with ollama gpt-oss:20b revision."""
import json
import logging
import sys
import time
...
def main():
    ...
    config = SoloSimulatorConfig(
        ...
        active_annotation_count=400,
        task_description=(
            "Classify the sentiment of the movie review as positive or negative. "
            "Labels: positive, negative"
        ),
        schema_name='sentiment',
        max_wait_autonomous=60,
        annotation_delay=3.0,  # slower so ollama cycles can complete mid-run
        wait_for_predictions_timeout=300,
    )

    sim = SoloModeSimulator(
        server_url='http://localhost:8511',
