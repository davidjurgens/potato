#!/usr/bin/env python3
"""Smoke runner: agent simulator against coding-agent-evaluation.

Tier-2 smoke for the coding-agent extension. Uses the new
`structured_turns` rendering in AgentSimulatorStrategy to surface the
agent's tool calls (Read/Edit/Bash/...) and outputs to the rater LLM.
...
CONFIG_PATH = REPO_ROOT / 'examples' / 'simulator-configs' / 'simulator-coding-agent-eval-ollama.yaml'
ANNOTATION_OUTPUT = REPO_ROOT / 'examples' / 'agent-traces' / 'coding-agent-evaluation' / 'annotation_output'
SERVER_URL = os.environ.get('POTATO_SMOKE_SERVER', 'http://localhost:8524')
...
def main() -> int:
    ...
    config.users = [
        UserConfig(
            user_id=f'sim_coding_eval_{run_tag}_{i:02d}',
    ...
    print('=' * 60)
    print('CODING-AGENT EVALUATION SMOKE')
    print('=' * 60)
