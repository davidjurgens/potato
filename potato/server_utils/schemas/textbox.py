...
def _generate_textbox_layout_internal(annotation_scheme):
    ...
    schematic = f"""
    <form id="{escape_html_content(schema_name)}" class="annotation-form textbox shadcn-textbox-container" action="/action_page.php" data-annotation-id="{escape_html_content(str(annotation_scheme["annotation_id"]))}" data-annotation-type="text" data-schema-name="{escape_html_content(schema_name)}" {layout_attrs}>
            {get_ai_wrapper()}
