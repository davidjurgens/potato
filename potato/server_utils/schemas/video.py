...
def _generate_video_layout_internal(annotation_scheme):
    ...
    schematic = f"""
        <form id="{escape_html_content(annotation_scheme['name'])}" class="annotation-form video" data-annotation-type="video" data-schema-name="{escape_html_content(annotation_scheme['name'])}" action="javascript:void(0)" data-annotation-id="{escape_html_content(str(annotation_scheme.get("annotation_id", "")))}" {layout_attrs}>
                 {get_ai_wrapper()}
