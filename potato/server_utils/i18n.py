...
UI_LANG_DEFAULTS = {
    ...
    'iaa_scale_lower_label': 'Error and offset metrics (MAE, RMSE, mean offset):',
    'iaa_scale_lower_note': 'lower is better; 0 means perfect agreement. Not colour-banded, because the strong/weak bands would be inverted.',
    'iaa_scale_distribution_label': 'Kolmogorov–Smirnov statistic:',
    'iaa_scale_distribution_note': '0 to 1; how far the within-item offset distribution sits from the between-item one. A separation measure, not an agreement coefficient.',
    'iaa_scale_coverage_label': 'Coverage:',
    'iaa_scale_coverage_note': '0 to 1; the share of the task that was answered at all. Not agreement — full coverage says everyone answered, not that they answered the same. Read it as the denominator behind the scores above.',
    'iaa_sweep_title': 'Sensitivity to the matching window',
    'iaa_sweep_note': 'Every measure below depends on how far apart two marks may be and still count as the same event. The highlighted row is the headline setting; read down a column to see whether agreement survives a tighter window.',
    'iaa_sweep_headline': 'headline',
    ...
    # Agreement drift / re-calibration prompt (iaa.html).
    'iaa_drift_title': 'Agreement over time',
    'iaa_drift_note': 'Windows are ordered batches of items, placed by when their last annotator finished. Each window is scored with the same metric as the project as a whole.',
    'iaa_drift_recalibrate': 'Re-calibration suggested',
    'iaa_drift_below_baseline': 'below the project baseline',
    'iaa_drift_baseline': 'Baseline',
    'iaa_drift_th_window': 'W',
    'iaa_drift_sparse': 'too few to judge',
    'iaa_drift_codebook_changes': 'Codebook revisions:',
    'iaa_drift_approx': 'approximate',
    'iaa_drift_untimed': 'item(s) carry no timestamp and are not on the timeline. This is expected for annotations imported from another tool.',
