...
def main(args=None) -> int:
    ...
    if not parsed.input:
        print("error: --input is required (or use --list-formats)",
              file=sys.stderr)
        return 2
    ...
    if not os.path.isfile(parsed.input):
        print(f"error: no such file: {parsed.input}", file=sys.stderr)
        return 2
    ...
    with open(parsed.input) as f:
        try:
            ...
            print(f"error: {parsed.input} is not valid JSON: {exc}",
                  file=sys.stderr)
            return 2
    ...
    if not fmt:
        ...
        print(f"error: could not detect the format of {parsed.input}. "
              f"Pass --input-format (one of: {supported}).", file=sys.stderr)
        return 2
    ...
    if os.path.exists(readme_path):
        ...
        _write_readme(readme_path, "config.yaml", result, parsed.input)
    ...
    print(f"Imported {result.stats['num_images']} image(s), "
          f"{result.stats['num_annotations']} annotation(s), "
          f"{result.stats['num_categories']} categor(ies) "
