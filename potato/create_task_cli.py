...
def get_annotation_type():
    while "the answer is invalid":
        ...
        q = 'What type of annotation is this? Possible types are...\n' + \
...
def create_task_cli():
    ...
    config['annotation_task_name'] = str(input('What is the name for your annotation task' +
                                               ' that will be shown to users?\n'))

    while True:
        port = str(input('What port do you want the server to run on? '))
        try:
            port = int(port)
    ...
    data_files = []
    fname = str(input('What is the absolute path to one of your data files? '))
    data_files.append(fname)

    # Let the user entire more files
    while yes_or_no('Do you have more data files?'):
        fname = str(input('What is the absolute path to another data files? '))
        data_files.append(fname)
    config['data_files'] = data_files
    
    id_key = str(input('Which field/column in the data file is the item\'s ID? '))
    text_key = str(input('Which field/column in the data file is the item\'s text? '))
    context_key = str(input('(optional) Which field/column in the data file is the item\'s additional context? '))
    ...
    config['annotation_codebook_url'] = str(input('What is the URL for the annotation codebook? '))

    config['output_annotation_dir'] = str(input('What is the absolute path for the directory ' +
                                                'where the annotations should be written?\n'))

    config["output_annotation_format"] = str(input('What format do you want the annotations written in?\n'
    ...
    while True:
        ...
        desc = str(input('What description/question/instructions should annotators see for this?\n'))
        name = str(input('What is the internal name for this category (used in output files)?\n'))        
        ...
        if atype == 'likert' or atype == 'bws':
            ...
            if atype == 'likert':
                size = str(input('How many items are on the likert scale?\n'))
            else:
                size = str(input('How many items to dislay at once for Best-Worst Scaling?\n'))
            ...
            # Get the options
            labels = []
            label = str(input('Enter the text for one option (or press enter with no input when done): '))
            while label != '':
                labels.append(label)
                label = str(input('Enter the text for one option (or press enter with no input when done): '))
    ...
    while True:
        config_file = str(input('What is the absolute path for where this config.yaml file should be written?\n'))
