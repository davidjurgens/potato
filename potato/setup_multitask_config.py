...
def main():
    ...
    if "multitask_guideline_file" in multitask_config and os.path.exists(multitask_config["multitask_guideline_file"]):
        ...
        for i, row in multitask_guideline_df.iterrows():
            ...
            for task in multitask_config["tasks"]:
                key2text[row['key']][task] = row[task] if type(row[task]) == str else row[multitask_config["base_task"]]
    ...
    for task in multitask_config["tasks"]:
        ...
        with open(multitask_config["base_config_file"], 'rt') as f:
            ...
            surveyflow_output_path = multitask_config["surveyflow_output_path"].replace("[TASK]", task)
            page = page.replace(multitask_config["surveyflow_path"], surveyflow_output_path + task + '-')
            page = page.replace('[TASK]', task)

            config = yaml.safe_load(page)

        config["output_annotation_dir"] = multitask_config['output_annotation_dir'].replace("[TASK]", task)
        ...
        config["annotation_task_name"] = multitask_config['annotation_task_name'].replace("[TASK]", task)
        config["data_files"] = [it.replace("[TASK]", task) for it in multitask_config['data_files']]
